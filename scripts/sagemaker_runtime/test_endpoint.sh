...
INPUT_BUCKET="${INPUT_BUCKET:-${TEST_INPUT_BUCKET:-${ASYNC_IO_BUCKET:-}}}"
OUTPUT_BUCKET="${OUTPUT_BUCKET:-${ASYNC_IO_BUCKET:-${INPUT_BUCKET}}}"
IO_BUCKET="${IO_BUCKET:-${ASYNC_IO_BUCKET:-${OUTPUT_BUCKET}}}"
TEST_IMAGE="${TEST_IMAGE:-inputs/test_image.png}"
JOB_ID="test-$(date +%s)"

if [ -z "${INPUT_BUCKET}" ]; then
    echo "ERROR: INPUT_BUCKET is required (or set TEST_INPUT_BUCKET / ASYNC_IO_BUCKET)." >&2
    exit 1
fi

if [ -z "${OUTPUT_BUCKET}" ]; then
    echo "ERROR: OUTPUT_BUCKET is required (or set ASYNC_IO_BUCKET)." >&2
    exit 1
fi

if [ -z "${IO_BUCKET}" ]; then
    echo "ERROR: IO_BUCKET is required (or set ASYNC_IO_BUCKET)." >&2
    exit 1
fi
...
if [ "$STATUS" != "InService" ]; then
    ...
    echo "Wait for endpoint to be ready or run scripts/sagemaker_runtime/setup_endpoint.sh"
    exit 1
fi
...
"${ROOT_DIR}/.venv/bin/python3" "${ROOT_DIR}/scripts/sagemaker_runtime/submit_job.py" \
